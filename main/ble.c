...
#define TEST_DEVICE_NAME "ESP_GATTS_DEMO2"
...
char msg[ESP_GATT_MAX_ATTR_LEN] = {0};
uint8_t msgIndex = 0;

uint32_t time5, time6;
...
static void __attribute__((unused)) HRS_IDX_BOBY_SENSOR_LOC_VAL(void)
{
    int dev_num = esp_ble_get_bond_device_num();
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  ///the app will receive this evt when the IO  has Output capability and the peer device IO has Input capability.
        ...
        sprintf(str_passkey, "%06ld", param->ble_security.key_notif.passkey);

        drawPasskey(120, str_passkey, convert_colordata(0x207F));
        break;
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT:
            ...
            char temp_value[18] = {0};

            if(msgIndex + param->write.len < ESP_GATT_MAX_ATTR_LEN)
            {
                memcpy(temp_value, param->write.value, param->write.len);
                strncat(msg, temp_value, param->write.len);
                memset(temp_value, 0, sizeof(temp_value));

                msgIndex += param->write.len;
            }            
            ...
            if(param->write.value[param->write.len-1] == '|')
            {
                ...
                ESP_LOGI(GATTS_TABLE_TAG, "msg: %s", msg);
                time5 = xTaskGetTickCount();
                parseCMD(msg, convert_colordata(PARSE_CMD_COLOR));
                time6 = xTaskGetTickCount();
                ESP_LOGI(GATTS_TABLE_TAG, "time5: %ld, time6: %ld", time5, time6);
                memset(msg, 0, sizeof(msg));       
            } 
            break;
        case ESP_GATTS_EXEC_WRITE_EVT:
            ESP_LOGI(GATTS_TABLE_TAG, "Here_3");
            break;
        ...
        case ESP_GATTS_CONNECT_EVT:
            ...
            // gpio_isr_handler_remove(UP_BTN);
            /* start security connect with peer device when receive the connect event sent by the master */
            esp_ble_set_encryption(param->connect.remote_bda, ESP_BLE_SEC_ENCRYPT_MITM);
            
            memcpy(conn_params.bda, param->connect.remote_bda, sizeof(esp_bd_addr_t));
            conn_params.latency = 0;
            conn_params.max_int = 0x50;    // 20 : 40ms, 1c : 35ms, 50 : 100ms, a0 : 200ms, 320 : 1000ms
            conn_params.min_int = 0x50;    // min_int = 0x10*1.25ms = 20ms
